...
void BMP085::calcTrueTemperature(){
  ...
  //read Raw Temperature
	  ...
	  delay(5);                                         // min. 4.5ms read Temp delay--> substituido pelo millis
...
//Com retorno
uint8_t BMP085::fast_calcTrueTemperature(){
  long ut,x1,x2;
  static uint8_t estado;
  static unsigned long t_amostra;
  //read Raw Temperature
  if (estado != AMOSTRA_PEDIDA)
  {
	  estado = AMOSTRA_PEDIDA;
	  writemem(CONTROL, READ_TEMPERATURE);
	  t_amostra = millis();
  }
  if (( millis() - t_amostra) >= 5)
  {
...
uint8_t BMP085::fast_calcTruePressure(long *_TruePressure) {
  long up,x1,x2,x3,b3,b6,p;
  unsigned long b4,b7;
  int32_t tmp; 
  static uint8_t estado;
  static unsigned long t_amostra;
	...
	{
		  #if AUTO_UPDATE_TEMPERATURE
		  if (fast_calcTrueTemperature()!= AMOSTRA_PRONTA)
		  {
			  return AGUARDANDO_AMOSTRA;
			  // b5 update
		  }
	  ...
	  if (( millis() - t_amostra) >= _pressure_waittime[_oss])
	  {
		  //delay(_pressure_waittime[_oss]);
		  readmem(CONTROL_OUTPUT, 3, _buff);
		  up = ((((long)_buff[0] <<16) | ((long)_buff[1] <<8) | ((long)_buff[2])) >> (8-_oss)); // uncompensated pressure value
